...
- **Anti-circularity design for MetS:** When validating BMN against the MetS outcome, MetS was excluded from the comorbidity inputs to prevent circular validation. This ensures that the AUC reflects genuine predictive ability from constituent clinical and biological variables, not from using the outcome as an input.
- AUC-ROC with 2,000 bootstrap replicates, Rubin's rules for combining multiply-imputed estimates
- NRI and IDI vs. baseline logistic regression (BMI + age + sex), Brier score, Hosmer-Lemeshow test with complementary ICI/E/O metrics
...
*Anti-circularity note:* The MetS prediction AUC was computed with MetS excluded from the comorbidity inputs to prevent circular validation (using the outcome as an input variable). When MetS is included as a comorbidity, the apparent AUC is inflated. All reported MetS AUCs use the anti-circularity design.

*NRI/IDI reference model:* Net Reclassification Improvement and Integrated Discrimination Improvement were computed against a logistic regression baseline using BMI + age + sex (not the BMN score itself) as predictors.

The BMN v3.5 outperforms established risk scores validated against their respective outcomes: FINDRISC for diabetes screening (AUC 0.72–0.81^3^), FRS for cardiovascular events (0.75–0.80^4^), SCORE2 for cardiovascular mortality (0.71–0.78^5^). *Note: These scores predict different endpoints and direct numerical comparison should be interpreted with caution.* When compared against machine learning classifiers trained on the same NHANES features (5-fold cross-validated), the BMN v3.5 shows lower discrimination than Random Forest and Gradient Boosting (AUC ~0.95; Supplementary Figure S4). This gap reflects a deliberate design trade-off: the BMN prioritises interpretability, partial-data compatibility, and therapeutic output generation over maximal discriminative performance—capabilities that ensemble ML models do not provide (Box 1). *Note: The BMN uses expert-derived weights (not data-fitted), so its AUC is inherently out-of-sample, whereas ML models used 5-fold cross-validation. The comparison is therefore methodologically fair.*

> **Box 1. Interpretable Clinical Score vs. Machine Learning Models: A Framework Comparison**
>
> The Score BMN v3.5 (AUC = 0.876) does not attempt to maximise discrimination at the expense of interpretability. Random Forest and Gradient Boosting models, trained via 5-fold cross-validation on the same NHANES features, achieve higher AUC (~0.95) but present three limitations precluding direct clinical deployment: (1) *black-box inference* — individual predictions cannot be decomposed into actionable clinical drivers; (2) *complete-data dependency* — ensemble models require all input features at inference, whereas the BMN operates with imputed or partially missing data via its Monte Carlo pipeline; and (3) *no therapeutic output* — ML models predict MetS presence but do not generate GRS profiles or BTM recommendations. The BMN v3.5 occupies a distinct design space: it is an interpretable, clinically deployable decision-support tool rather than a prediction-optimised classifier. The comparison is methodologically fair: BMN weights are expert-derived (inherently out-of-sample), while ML models are cross-validated. Future hybrid architectures integrating ML-derived weights into the BMN framework are a planned development direction (§4.11).

**Temporal validation:** In the independent temporal validation cohort (2015–2018, N ≈ 11,500), the BMN v3.5 was applied without retraining to data chronologically subsequent to the development cohort (2011–2014, N ≈ 11,300). *[Exact AUC values will be populated from the pipeline output after re-running with anti-circularity corrections applied.]*

**Calibration:** The Hosmer-Lemeshow test indicated significant miscalibration (p<0.001), attributable to the large sample size amplifying minor calibration deviations—a known property of the HL test at N>10,000.^17^ Complementary calibration metrics were computed: Integrated Calibration Index (ICI), Expected/Observed (E/O) ratio, and calibration slope from logistic recalibration. *[Values will be populated from the corrected pipeline output.]* Note that the BMN sf/100 is a risk score, not a calibrated probability; calibration metrics assess the monotonic relationship between score and outcome frequency.
